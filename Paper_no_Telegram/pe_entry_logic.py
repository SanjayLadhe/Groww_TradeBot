...
This module handles the entry logic for Put options (bearish trades).
...
def check_pe_entry_conditions(
    df: pd.DataFrame,
    symbol: str,
    tsl: Any
) -> Dict[str, Any]:
    """
    Check if PE entry conditions are met.

    Entry Conditions for PE (Bearish):
    1. Price below VWAP
    2. -DI > +DI (bearish directional movement)
    3. ADX > 23 (trending market)
    4. ADX rising (momentum building)
    5. Price below lower Fractal Chaos Band (breakdown confirmation)
    ...
    try:
        ...
        # Condition 1: Price below VWAP (bearish bias)
        if latest['close'] >= latest['vwap']:
            return {"valid": False, "reason": "Price above VWAP"}

        # Condition 2: -DI > +DI (bearish directional movement)
        if latest['minus_di'] <= latest['plus_di']:
            return {"valid": False, "reason": "Bullish DI crossover"}
        ...
        if 'lower_band' in df.columns:
            if latest['close'] > latest['lower_band']:
                return {"valid": False, "reason": "Above lower band"}
        ...
        logger.info(f"PE Entry Signal: {symbol}")
        ...
        logger.error(f"Error checking PE entry for {symbol}: {e}")
...
def execute_pe_entry(
    ...
    """
    Execute PE (Put) entry trade.

    Args:
        tsl: Trading instance
        symbol: Underlying symbol
        df: DataFrame with indicator data
        signal: Entry signal from check_pe_entry_conditions
    ...
    """
    try:
        logger.info(f"Executing PE Entry for {symbol}")

        # Get spot price
        spot_price = signal['close']

        # Get option details
        option_info = get_option_details(tsl, symbol, spot_price, "PE")
        ...
        logger.info(f"PE Order Details:")
        ...
        if order_result["status"] == "success":
            ...
            logger.info(f"PE Order Executed: {order_id} @ {executed_price}")
        ...
        logger.error(f"PE entry execution error: {e}")
